...
module Convert =
    ...
    let isUsingFable3 = true

    let internal insideWorker :  bool = false
    ...
    let rec fromJsonAs (input: Json) (typeInfo: Fable.SimpleJson.Python.TypeInfo) : obj =
        ...
        | JString serializedRecord, TypeInfo.Record getFields ->
            fromJsonAs (SimpleJson.parseNative serializedRecord) typeInfo
    ...
    // TODO: reimplement from "./quote"
    let quoteText (inputText: string) : string = betweenQuotes inputText
    ...
    let rec serialize value (typeInfo: TypeInfo) =
        ...
        | TypeInfo.Object ->
            Fable.Python.Json.json.dumps(value)
